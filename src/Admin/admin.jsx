...
export default function Admins() {
  ...
  async function getAdmins() {
    try {
      const response = await axios.get(`${Middleware.baseURL}/admins`, {
        headers: Middleware.header,
      });
  ...
  const rows = admins.map((item, itemIndex) => {
    return {
      id: item._id,
  ...
  const modals = admins.map((item) => {
    return [
      ...
      >
        ...
        <Modal.Body>Areyou sure you want delete this admin?</Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => {
